...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { service_key, input, user_id } = await req.json()
    ...
    // A. Try getting user from Auth (Bearer Token)
    const { data: { user }, error: userError } = await supabaseClient.auth.getUser()
    if (user && !userError) {
        targetUserId = user.id
    }

    // B. If Service Role Key is used, allow override via body
    if (!targetUserId && user_id) {
        // Verify if the request is using Service Role Key
        const serviceRoleKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')
        const token = authHeader.replace('Bearer ', '')
        
        if (token === serviceRoleKey) {
            targetUserId = user_id
            console.log(`[AiGateway] Service Role invoked. Impersonating user: ${targetUserId}`)
        } else {
             console.warn(`[AiGateway] user_id provided but not using Service Role Key. Ignoring.`)
        }
    }
    ...
    // If we are impersonating (targetUserId determined via Service Role), 
    // we MUST use supabaseAdmin to fetch user's config because supabaseClient (Service Role) 
    // might not respect RLS in the way we want (it bypasses everything), 
    // BUT we need to query tables that might be protected.
    // Actually, Service Role bypasses RLS, so we can just query llm_configs directly filtering by user_id if needed.
    // However, the original code used supabaseClient (which was either user-authed or service-role-authed).
    
    // Strategy:
    // If targetUserId is set (either from token or impersonation), we want to fetch the config.
    // If it was a User Token, supabaseClient has the user context.
    // If it was Service Role + user_id, supabaseClient is admin.
    
    // Let's stick to using supabaseClient but ensure we query correctly.
