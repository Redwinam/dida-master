...
export default defineEventHandler(async (event) => {
  console.log('[DailyNote] Request received')
  let config: UserConfig

  // 1. Load Config & Validate
  try {
    config = await getUserConfig(event) as unknown as UserConfig
    ...
    console.error('[DailyNote] Config Error:', e)
    return {
      error: true,
      message: e.message,
  ...
  const processDailyNote = async () => {
    try {
      console.log('[DailyNote] Starting background processing...')
      ...
      try {
        ...
        throw new Error(`Dida Projects Fetch Failed: ${e}`)
      }
      ...
      console.log('[DailyNote] Note created successfully')

    } catch (e: any) {
      console.error('[DailyNote] Fatal Background Error:', e)
    }
  }

  // 3. Execute Background Task
  if (event.waitUntil) {
    event.waitUntil(processDailyNote())
  } else {
    // Fire and forget for environments without waitUntil
    processDailyNote().catch(e => console.error('Background task failed:', e))
  }

  // 4. Return Immediate Response
  return { 
    message: 'Success',
    details: 'Daily note generation started in background. Check logs for progress.'
  }
})
